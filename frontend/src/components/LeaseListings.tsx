...
export default function LeaseListings() {
  ...
  const { data: listings, isLoading, error, isFetching, dataUpdatedAt } = useGetAllLeaseListings();
    ...
    return (
      <Card>
        <CardContent className="pt-6 pb-6">
          <p className="text-center text-muted-foreground text-lg">No leases available yet — check back soon!</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-end gap-2 mb-4 text-xs text-muted-foreground">
        ...
        <span>{isFetching ? 'Refreshing…' : lastUpdated ? `Updated ${lastUpdated}` : 'Live'}</span>
      </div>
            ...
            <Card key={listing.listingId.toString()} className={!isAvailable ? 'opacity-75' : ''}>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between gap-2">
                  <CardTitle className="text-base sm:text-lg">Lease #{listing.listingId.toString()}</CardTitle>
                  <Badge
              ...
              <CardContent className="space-y-4">
                <div>
                  <p className="text-xs sm:text-sm text-muted-foreground mb-1">Lease Code</p>
                  <p className="font-mono text-sm bg-muted px-3 py-2 rounded break-all">
                    {formatPartialCode(listing.leaseCode)}
                  </p>
                </div>
                <div>
                  <p className="text-xs sm:text-sm text-muted-foreground mb-1">Split Ratio</p>
                  <p className="font-semibold text-base sm:text-lg">{listing.splitRatio}</p>
                </div>
                {!isAvailable && (
